...
export default function QuestionRepository() {
  ...
  const loadQuestions = useCallback(async () => {
    ...
    setLoading(false);
  }, [page, perPage, debouncedSearch, selectedSubject, selectedDifficulty, activeOnly]);
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-4 md:space-y-6 animate-fadeIn">
      ...
      {/* Top Filters */}
      <div className="ncc-glass-card p-4 space-y-3">
        <div className="flex flex-col lg:flex-row gap-3 items-stretch lg:items-center justify-between">
          ...
          <div className="flex gap-2 flex-wrap">
            <label className="flex items-center gap-2 text-xs font-medium text-navy-900 cursor-pointer bg-surface-50 px-3 py-2 rounded-lg border border-surface-200">
              <input type="checkbox" checked={activeOnly} onChange={e => { setActiveOnly(e.target.checked); setPage(1); }} className="w-4 h-4 accent-gold-500" />
              Active Only
            </label>
            <button onClick={() => setCsvModalOpen(true)} className="ncc-btn ncc-btn-ghost py-2 px-4 cursor-pointer text-sm">
              <Upload className="w-4 h-4" /> Import CSV
            </button>
            <button onClick={() => { setEditingQuestion(null); setIsModalOpen(true); }} className="ncc-btn ncc-btn-primary py-2 px-4 cursor-pointer text-sm">
