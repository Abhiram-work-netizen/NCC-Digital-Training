...
export default function Profile() {
  ...
  const [form, setForm] = useState({ full_name: '', ncc_number: '', wing: '', certificate_level: '' });
  ...
  useEffect(() => {
    ...
    const loadStats = async () => {
      ...
      const { data: attempts } = await supabase.from('test_attempts')
        .select('score').eq('user_id', user.id).in('status', ['submitted', 'flagged']);
  ...
  const handleSave = async () => {
    if (!user) return;
    setSaving(true);
    const { error } = await supabase.from('cadet_profiles').update({
